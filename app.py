...
<div class="sticky-nav">
    <div style="display:flex;align-items:center;">
        ...
        <span class="nav-subtitle">3-Phase Induction Motor &nbsp;&middot;&nbsp; Predictive Maintenance</span>
        <span class="nav-badge">Random Forest</span>
    </div>
    <div class="nav-right">
        <span class="nav-stat">R&sup2; {stats['rul_r2']:.4f}</span>
        <span class="nav-stat">Acc {stats['fail_acc']*100:.1f}%</span>
        <span class="nav-stat">{stats['n_samples']:,} samples</span>
        <span class="nav-stat" style="color:{verdict_color};">{verdict_label}</span>
    </div>
</div>
...
with left_col:
    ...
    st.markdown('<div class="panel-title">Sensor Input Panel</div>', unsafe_allow_html=True)

    machine_type = st.selectbox("Machine Type", ["L", "M", "H"], index=1)

    st.markdown('<div class="panel-section-label">Thermal Sensors</div>', unsafe_allow_html=True)
    ...
    st.markdown('<div class="panel-section-label">Mechanical Sensors</div>', unsafe_allow_html=True)
    ...
    st.markdown('<div class="panel-section-label">Degradation Rate</div>', unsafe_allow_html=True)
...
with right_col:
    ...
    with eda_tab3:
        ...
        with fail_col2:
            ...
            fig_fbw.update_layout(
                ...
                font=dict(color='#7d8590', family='DM Sans'),
                xaxis=dict(gridcolor='#21262d', tickfont=dict(family='DM Mono', size=10)),
                yaxis=dict(gridcolor='#21262d', title='Failure Rate (%)', tickfont=dict(family='DM Mono', size=10)),
                margin=dict(l=10, r=10, t=40, b=10), height=280, showlegend=False,
            )
    ...
    if len(cm) == 2:
        ...
        fig_cm = go.Figure(data=go.Heatmap(
            ...
            textfont=dict(size=14, family='DM Mono', color='#e6edf3'),
            showscale=False,
        ))
